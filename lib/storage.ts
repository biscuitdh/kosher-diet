...
export type LocalDataChangeDetail =
  ...
  | { type: "grocery-clear-checked"; profileId: string }
...
export function clearCheckedGroceryItems(profileId = getSelectedRecipeProfileId()) {
  void profileId;
  saveGroceryItems(loadAllGroceryItems().filter((item) => !item.checked));
  emitLocalDataChange({ type: "grocery-clear-checked", profileId: DEFAULT_RECIPE_PROFILE_ID });
}
