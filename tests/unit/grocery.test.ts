...
import { suggestGroceryItems } from "@/lib/grocery-suggestions";
import {
...
describe("grocery helpers", () => {
  ...
  it("suggests catalog grocery items with quantity and unit while typing", () => {
    const suggestions = suggestGroceryItems("call");

    expect(suggestions[0]).toMatchObject({
      displayName: "Cauliflower rice",
      quantity: "3",
      unit: "cups",
      source: "catalog"
    });
  ...
  it("prefers matching items already on the grocery list", () => {
    const carrots = groceryItem("Carrots", "2", "items");
    const suggestions = suggestGroceryItems("car", [carrots]);
  ...
  it("clears checked groceries with deletion ids and ingredient keys for cloud sync", () => {
    const changeSpy = vi.fn();
    window.addEventListener(LOCAL_DATA_CHANGED_EVENT, changeSpy as EventListener);

    const result = addCustomGroceryItem({ displayName: "Cauliflower rice", quantity: "3", unit: "cups" }, "household");
    updateGroceryItem({ ...result.item, checked: true });
    clearCheckedGroceryItems("household");

    const clearEvent = changeSpy.mock.calls
      .map(([event]) => event as CustomEvent)
      .find((event) => event.detail?.type === "grocery-clear-checked");

    expect(loadGroceryItemsForProfile("household")).toHaveLength(0);
    expect(clearEvent?.detail).toMatchObject({
      type: "grocery-clear-checked",
      ids: [result.item.id],
      ingredientKeys: ["cauliflower rice"]
    });

    window.removeEventListener(LOCAL_DATA_CHANGED_EVENT, changeSpy as EventListener);
  });

  it("restores cleared grocery items and emits upsert events for sync", () => {
    const changeSpy = vi.fn();
    window.addEventListener(LOCAL_DATA_CHANGED_EVENT, changeSpy as EventListener);

    const apples = groceryItem("Apples", "6", "items");
    const carrots = groceryItem("Carrots", "2", "cups");
    restoreGroceryItems([apples, carrots]);

    expect(loadGroceryItemsForProfile("household").map((item) => item.displayName)).toEqual(["Apples", "Carrots"]);
    expect(
      changeSpy.mock.calls
        .map(([event]) => event as CustomEvent)
        .filter((event) => event.detail?.type === "grocery-upsert")
        .map((event) => event.detail.item.displayName)
    ).toEqual(["Apples", "Carrots"]);
