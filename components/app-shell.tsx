...
import { BookOpen, ChefHat, Heart, Search, ShoppingCart } from "lucide-react";
...
function HeaderRecipeSearch() {
  ...
  return (
    <form
      ...
      className="relative order-3 flex w-full min-w-0 gap-2 xl:order-2 xl:max-w-[min(44vw,52rem)] xl:flex-1"
      onSubmit={submit}
      role="search"
    >
          ...
          >
            {suggestions.map((title) => (
              <button
                ...
                role="option"
                aria-selected="false"
                className="block w-full px-3 py-2 text-left text-sm hover:bg-secondary focus-ring"
  ...
  return (
    <div className="min-h-dvh bg-[radial-gradient(circle_at_top_left,rgba(231,161,87,0.16),transparent_32rem),linear-gradient(180deg,hsl(var(--background)),hsl(var(--background)))]">
      <header className="sticky top-0 z-40 border-b bg-background/88 backdrop-blur">
        <div className="container flex min-h-16 flex-wrap items-center justify-between gap-3 py-3 xl:flex-nowrap">
          <Link href="/" className="order-1 mr-auto flex min-w-0 items-center gap-2 font-semibold xl:mr-0" aria-label={`${APP_NAME} home`}>
          ...
          <div className="order-2 flex shrink-0 items-center gap-2 xl:order-4">
            <ThemeToggle />
            <HeaderProfileControl />
          </div>

          <Suspense fallback={<div className="order-3 h-11 w-full xl:order-2 xl:max-w-[min(44vw,52rem)] xl:flex-1" />}>
            <HeaderRecipeSearch />
          </Suspense>

          <nav className="order-3 hidden shrink-0 items-center gap-1 xl:flex" aria-label="Primary navigation">
            {navItems.map((item) => {
