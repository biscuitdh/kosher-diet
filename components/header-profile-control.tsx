...
export function HeaderProfileControl() {
  ...
  const localOnly = Boolean(session && !syncConfigured);
  const displayName = mounted && localOnly ? "Local dev" : mounted && session?.user.email ? session.user.email : "Sign in";
  const accountLabel = mounted && localOnly
    ? "KosherTable local development mode"
    : mounted && session?.user.email
      ? `KosherTable account: ${session.user.email}`
      : "Sign in to KosherTable";
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {localOnly ? "Local development mode stores favorites and groceries in this browser." : "Sign in once and use the same favorites and groceries on every device."}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="rounded-lg border bg-secondary/35 p-3 text-sm">
            {session ? (
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="font-medium">{localOnly ? "Local development mode" : `Signed in as ${session.user.email ?? "signed-in user"}`}</p>
                  <p className="text-xs text-muted-foreground">
                    {localOnly
                      ? "Google sign-in and cloud sync are disabled for this localhost session."
                      : syncStatus === "syncing"
                        ? "Syncing favorites and groceries..."
                        : "Favorites and groceries sync across devices."}
                  </p>
                </div>
                {localOnly ? null : (
