...
Only signed-in, email-verified Google accounts with matching `allowed_users/{email}` documents can open the app or read/write the shared household favorites and groceries. Terraform creates those docs from `allowed_emails`.
...
3. Confirm no recipe UI renders before Google sign-in.
4. Sign in with an allowed Google account.
5. Favorite a recipe.
6. Add recipe ingredients to groceries.
7. Refresh on the other device and confirm favorites/groceries sync.
8. Try a non-whitelisted email and confirm it cannot open the app.
9. Confirm Google Cloud Billing shows the `KosherTable monthly budget` alert at `$5`.
